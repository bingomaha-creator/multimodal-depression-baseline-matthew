...
from src.datasets.edaic_segment_feature_dataset import (
    EDAICSegmentFeatureDataset,
    collate_edaic_segment_features,
)
from src.models.edaic_segment_feature_baseline import EDAICSegmentFeatureBaseline
from src.utils.metrics import classification_metrics, find_best_threshold
from src.utils.seed import set_seed


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train E-DAIC timestamp-aligned segment feature baseline.")
...
def labels_from_dataset(dataset: EDAICSegmentFeatureDataset) -> np.ndarray:
...
def validate_split_labels(datasets: Dict[str, EDAICSegmentFeatureDataset]) -> None:
    for split, dataset in datasets.items():
        labels = labels_from_dataset(dataset)
        unique, counts = np.unique(labels, return_counts=True)
        label_counts = dict(zip(unique.tolist(), counts.tolist()))
        segment_counts = [int(item["num_segments"]) for item in dataset.items]
        print(
            f"{split} label counts: {label_counts}; "
            f"segments min={min(segment_counts)} max={max(segment_counts)} "
            f"mean={sum(segment_counts) / len(segment_counts):.2f}"
        )
...
def build_dataloaders(
    ...
    datasets = {
        "train": EDAICSegmentFeatureDataset(feature_cache_path, split="train"),
        "dev": EDAICSegmentFeatureDataset(feature_cache_path, split="dev"),
        "test": EDAICSegmentFeatureDataset(feature_cache_path, split="test"),
    }
    ...
    if overfit_small is not None:
        ...
        datasets = {
            split: EDAICSegmentFeatureDataset(feature_cache_path, indices=indices, split="train")
    ...
    return {
        split: DataLoader(
            ...
            collate_fn=collate_edaic_segment_features,
        )
        for split, dataset in datasets.items()
    }


def build_model(metadata: Dict[str, Any], model_cfg: Dict[str, Any], modality: str) -> EDAICSegmentFeatureBaseline:
    return EDAICSegmentFeatureBaseline(
...
def evaluate(
    ...
    for batch in tqdm(loader, desc="evaluate", leave=False):
        ...
        for participant_id, split, phq_score, num_segments, label, pred, prob in zip(
            batch["participant_id"],
            batch["split"],
            batch["phq_scores"].detach().cpu().numpy().tolist(),
            batch["num_segments"].detach().cpu().numpy().tolist(),
            batch_labels,
            batch_preds,
            batch_probs,
        ):
