...
On the server, first validate all 961 samples, feature dimensions, finite values, per-modality sequence lengths, and official split counts. Length mismatches are reported but preserved because both baselines encode or pool each modality independently before fusion:
